...
public class BigHunter extends MonsterEntity {
    private int attackTimer;

    public BigHunter(EntityType<? extends BigHunter> type, World worldIn) {
        super(type, worldIn);
        this.noCulling = true;
    }
    ...
    public static AttributeModifierMap.MutableAttribute registerAttributes() {
        return MobEntity.createMobAttributes()
                ...
                .add(Attributes.MAX_HEALTH, BigHunterHp);
    }
    ...
    public void tick() {
        super.tick();
        PlayerEntity target = this.level.getNearestPlayer(this, BigHunterDistance);
        if (target != null) {
            if (this.canSee(target)) {
                // 視界にプレイヤーがいる場合は追跡する
                this.getNavigation().moveTo(target, BigHunterSpeed);
